...
import BienDongDialog from '../components/biendong/BienDongDialog';
...
import { khaiTu, chuyenDi } from '../api/bienDongApi';
...
export default function NhanKhauPage() {
    ...
    // Thêm state cho biến động
    const [bienDongDialog, setBienDongDialog] = useState<{
        open: boolean;
        type: 'khaiTu' | 'chuyenDi';
        nhanKhau: any | null;
    }>({ open: false, type: 'khaiTu', nhanKhau: null });
    ...
    // Thêm function xử lý biến động
    const handleKhaiTu = (nhanKhau: NhanKhau) => {
        setBienDongDialog({ open: true, type: 'khaiTu', nhanKhau });
    };

    const handleChuyenDi = (nhanKhau: NhanKhau) => {
        setBienDongDialog({ open: true, type: 'chuyenDi', nhanKhau });
    };

    const handleBienDongConfirm = async (data: any) => {
        try {
            if (bienDongDialog.type === 'khaiTu') {
                await khaiTu(bienDongDialog.nhanKhau.id, data);
                enqueueSnackbar('Khai tử thành công', { variant: 'success' });
            } else {
                await chuyenDi(bienDongDialog.nhanKhau.id, data);
                enqueueSnackbar('Chuyển đi thành công', { variant: 'success' });
            }
            setBienDongDialog({ ...bienDongDialog, open: false });
            loadNhanKhauData(); // Reload data
        } catch (error) {
            console.error('Error processing bien dong:', error);
            enqueueSnackbar('Không thể xử lý biến động', { variant: 'error' });
        }
    };
    ...
    return (
        <Box sx={{ width: '100%', maxWidth: '100%' }}>
                        ...
                        <Table sx={{ width: '100%', tableLayout: 'fixed' }} size="small">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell sx={{ fontWeight: 'bold', width: '10%' }}>Ngày sinh</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '5%' }}>Tuổi</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '8%' }}>Giới tính</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '10%' }}>CCCD</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '12%' }}>Nghề nghiệp</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '10%' }}>Quan hệ</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '8%' }}>Mã HK</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '8%' }}>Trạng thái</TableCell>
                                    <TableCell align="center" sx={{ fontWeight: 'bold', width: '8%' }}>Thao tác</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                    ...
                                    <TableRow key={nhanKhau.id} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                        ...
                                        <TableCell>
                                            <Chip
                                                ...
                                                size="small"
                                                color={nhanKhau.gioiTih === 'Nam' ? 'primary' : 'secondary'}
                                        ...
                                        <TableCell>
                                            {nhanKhau.trangThai && (
                                                <Chip
                                                    label={nhanKhau.trangThai}
                                                    size="small"
                                                    color={
                                                        nhanKhau.trangThai === 'DA_MAT' ? 'error' :
                                                            nhanKhau.trangThai === 'DA_CHUYEN_DI' ? 'warning' :
                                                                'default'
                                                    }
                                                    sx={{ fontSize: '0.7rem', height: 24 }}
                                                />
                                            ...
                                            <Stack direction="row" spacing={0.5} justifyContent="center">
                                                ...
                                                <Tooltip title="Thao tác khác">
                                                    <IconButton
                                                        ...
                                                        color="default"
                                                        onClick={(e) => {
                                                            // Here you would typically open a menu with more options
                                                            // For now, we'll just open the chuyen di dialog
                                                            handleChuyenDi(nhanKhau);
                                                        }}
                                                    >
            ...
            {/* Dialog biến động */}
            <BienDongDialog
                open={bienDongDialog.open}
                onClose={() => setBienDongDialog({ ...bienDongDialog, open: false })}
                onConfirm={handleBienDongConfirm}
                type={bienDongDialog.type}
                nhanKhau={bienDongDialog.nhanKhau}
            />
        </Box>
    );
}