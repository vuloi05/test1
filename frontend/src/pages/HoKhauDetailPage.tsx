...
import { khaiSinh } from '../api/bienDongApi';
...
import KhaiSinhForm from '../components/forms/KhaiSinhForm';
import LichSuBienDong from '../components/biendong/LichSuBienDong';
...
export default function HoKhauDetailPage() {
    ...
    const [openKhaiSinhForm, setOpenKhaiSinhForm] = useState(false);
    ...
    // Thêm function xử lý khai sinh
    const handleKhaiSinh = async (data: KhaiSinhRequest) => {
        if (!hoKhauId) return;
        try {
            await khaiSinh(parseInt(hoKhauId), data);
            // Reload danh sách nhân khẩu
            const nhanKhauData = await getDanhSachNhanKhau(parseInt(hoKhauId));
            setNhanKhauList(nhanKhauData);
            setOpenKhaiSinhForm(false);
        } catch (error) {
            console.error("Failed to khai sinh:", error);
        }
    };
        ...
        <>
            <Box sx={{ width: '100%', maxWidth: '100%' }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3, width: '100%' }}>
                    ...
                    {/* Cột danh sách Nhân khẩu với Tabs */}
                    <Box sx={{ width: { xs: '100%', md: '66.67%' } }}>
                        <Paper sx={{ p: 2, height: '100%', width: '100%' }}>
                            {/* Thêm Tabs vào giao diện */}
                            <Tabs value={tabValue} onChange={(e, v) => setTabValue(v)}>
                                <Tab label="Danh sách thành viên" />
                                <Tab label="Lịch sử biến động" />
                            </Tabs>
                            ...
                            {tabValue === 0 && (
                                <Box>
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', mt: 2 }}>
                                        ...
                                        <Box>
                                            <Button variant="contained" startIcon={<AddIcon />} onClick={handleOpenCreateForm} sx={{ mr: 1 }}>
                                                Thêm Thành viên
                                            </Button>
                                            <Button variant="outlined" onClick={() => setOpenKhaiSinhForm(true)}>
                                                Khai sinh
                                            </Button>
                                        </Box>
                                    </Box>
                            ...
                            {tabValue === 1 && (
                                <Box sx={{ mt: 2 }}>
                                    <LichSuBienDong type="hoKhau" id={parseInt(hoKhauId!)} />
                                </Box>
                            )}
                        </Paper>
                    </Box>
            ...
            />
            <KhaiSinhForm
                open={openKhaiSinhForm}
                onClose={() => setOpenKhaiSinhForm(false)}
                onSubmit={handleKhaiSinh}
                hoKhauId={parseInt(hoKhauId!)}
            />
        </>
    );
}